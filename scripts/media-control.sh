...
function get_brightness_icon {
    ...
    brightness=$(get_brightness_icon)
    if [ "$brightness" -lt 15 ]; then
        brightness_icon="󰃚 "
    elif [ "$brightness" -lt 29 ]; then
        brightness_icon="󰃛 "
    elif [ "$brightness" -lt 42 ]; then
        brightness_icon="󰃜 "
    elif [ "$brightness" -lt 57 ]; then
        brightness_icon="󰃝 "
    elif [ "$brightness" -lt 71 ]; then
        brightness_icon="󰃞 "
...
function show_volume_notif {
    ...
    if [[ $show_music_in_volume_indicator == "true" ]]; then
        ...
        dunstify -a "osd" -t $notification_timeout -h string:x-dunst-stack-tag:volume_notif -h int:value:$volume -i "$album_art" "$volume_icon $volume%" "$current_song"
    else
        # notify-send -t $notification_timeout -h string:x-dunst-stack-tag:volume_notif -h int:value:$volume "$volume_icon $volume%"
        dunstify -a "osd" -t $notification_timeout -h string:x-dunst-stack-tag:volume_notif -h int:value:$volume "$volume_icon $volume%"
    fi
}
...
function show_music_notif {
    ...
    dunstify -a "osd" -t $notification_timeout -h string:x-dunst-stack-tag:music_notif -i "$album_art" "$song_title" "$song_artist - $song_album"
}
...
function show_mic_notif {
    ...
    dunstify -a "osd" -t $notification_timeout -h string:x-dunst-stack-tag:mic_volume_notif -h int:value:$volume "$mic_icon $volume%"
}
...
function show_brightness_notif {
    ...
    dunstify -a "osd" -t $notification_timeout -h string:x-dunst-stack-tag:brightness_notif -h int:value:$brightness "$brightness_icon $brightness%"
}
